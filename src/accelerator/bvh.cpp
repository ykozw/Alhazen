...
-------------------------------------------------
*/
void SimpleBVH::constructNode(int32_t nodeIndex, MeshTriangle* triangles,
...
-------------------------------------------------
*/
bool QBVH::construct(const std::vector<Vec3>& vs, const std::vector<Vec3>& ns,
                     const std::vector<Vec2>& ts,
                     const std::vector<MeshFace>& fs) {
  // AL_ASSERT_DEBUG(vs.size() == ns.size());
  AL_ASSERT_DEBUG(fs.size() != 0);
...
void QBVH::constructNodeMedian(int32_t nodeIndex,
                               RefArray<QBVHBuildTriangle>& tris, int32_t depth,
                               int32_t& progress) {
  ...
  sortAxis = axisLR;
#if 1
  std::nth_element(lTris.begin(), lTris.begin() + lTris.size() / 2, lTris.end(),
                   sortPred);
  partitionValue = lTris[lTris.size() / 2].aabb.center()[sortAxis];
  std::partition(lTris.begin(), lTris.end(), partitionPred);
  std::nth_element(rTris.begin(), rTris.begin() + rTris.size() / 2, rTris.end(),
                   sortPred);
