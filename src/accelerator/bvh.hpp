...
class SimpleBVH : public BVHBase {
 ...
 private:
  void constructNode(int32_t nodeIndex, _In_ MeshTriangle* triangles,
                     int32_t numTriangle, int32_t depth);
  bool intersectSub(int32_t nodeIndex, const Ray& ray,
...
struct QBVHLeaf {
  ...
  // 法線 (t1x1, t1x2, t1x3) (t2x1, t2x2, t2x3) (t3x1, t3x2, t3x3) (t4x1, t4x2,
  // t4x3)
...
class QBVH {
 public:
  ...
  ~QBVH() {}
  bool construct(const std::vector<Vec3>& vs, const std::vector<Vec3>& ns,
                 const std::vector<Vec2>& ts, const std::vector<MeshFace>& fs);
  int32_t numVerts() const;
  int32_t numFaces() const;
  int32_t maxDepth() const;
  AABB aabb() const;
  bool intersect(const Ray& ray, _Out_ Intersect* isect,
 ...
 private:
  void constructNodeMedian(int32_t nodeIndex, RefArray<QBVHBuildTriangle>& tris,
                           int32_t depth, int32_t& progress);
  void constructNodeSAH(int32_t nodeIndex, RefArray<QBVHBuildTriangle>& tris,
                        int32_t depth, int32_t& progress);
