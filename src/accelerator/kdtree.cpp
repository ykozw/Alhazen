...
AL_TEST(Accel, kdtree0) {
  ...
  struct TestPoint {
   ...
   public:
    ...
    bool operator==(const TestPoint& other) const { return (pos == other.pos); }
    bool operator!=(const TestPoint& other) const { return !(*this == other); }
  };
  const auto rp = []() {
    const auto rv = []() { return float(rand() % 100) / float(100.0f) - 0.5f; };
  ...
  for (int32_t i = 0; i < 128; ++i) {
    ...
    std::sort(points.begin(), points.end(),
              [serch](const Point& lhs, const Point& rhs) {
                return Vec3::distance(serch, lhs) < Vec3::distance(serch, rhs);
              });
    //
