...
public:
    ...
    float findKNN(Vec3 point, int32_t numSerch, std::vector<const KdTreePoint*>& points) const;
    float findKNNBF(Vec3 poin, int32_t numSerch, std::vector<const KdTreePoint*>& points) const;
    void print() const;
private:
    int32_t constructSub(
        KdTreePoint* points,
        int32_t numPoints,
...
*/
template<typename KdTreePoint>
inline void KdTree<KdTreePoint>::construct(KdTreePoint* points, int32_t numPoints)
{
...
*/
template<typename KdTreePoint>
inline int32_t KdTree<KdTreePoint>::constructSub(
    KdTreePoint* points,
    int32_t numPoints,
    std::vector<NodeType>& nodes,
    int32_t& nodeIndex )
{
    ...
    // その軸で二分する
    std::nth_element(points, points + midPoint, points + numPoints,
    ...
    NodeType& node = nodes[thisNodeIndex]; ++nodeIndex;
    node.point = points[midPoint];
    node.left = constructSub(points, midPoint, nodes, nodeIndex);
    node.right = constructSub(points + midPoint + 1, numPoints - midPoint - 1, nodes, nodeIndex);
    node.axis = axis;
    // AABBの作成
...
*/
template<typename KdTreePoint>
INLINE KdTreePoint KdTree<KdTreePoint>::findNearest(Vec3 point, float* minDist) const
{
    // 空のツリーはそもそも探索しない
    ...
    std::vector<const KdTreePoint*>& points) const
{
    ...
    const auto setFarNodeToLast = [&]()
    {
        ...
        std::nth_element(points.begin(), points.begin() + points.size() - 1, points.end(),
            [&targetPos, &farDistSq, this](
                const KdTreePoint* lhs,
...
*/
template<typename KdTreePoint>
INLINE float KdTree<KdTreePoint>::calcDist(const NodeType& node, Vec3 pos) const
{
...
*/
template<typename KdTreePoint>
INLINE float KdTree<KdTreePoint>::calcDist(const KdTreePoint* point, Vec3 pos) const
{
...
*/
template<typename KdTreePoint>
INLINE float KdTree<KdTreePoint>::calcDistSq(const NodeType& node, Vec3 pos) const
{
...
*/
template<typename KdTreePoint>
INLINE float KdTree<KdTreePoint>::calcDistSq(const KdTreePoint* point, Vec3 pos) const
{
