...
class KdTree {
 ...
 public:
  ...
  float findKNN(Vec3 point, int32_t numSerch,
                std::vector<const KdTreePoint*>& points) const;
  float findKNNBF(Vec3 poin, int32_t numSerch,
                  std::vector<const KdTreePoint*>& points) const;
  void print() const;

 private:
  int32_t constructSub(KdTreePoint* points, int32_t numPoints,
                       std::vector<NodeType>& nodes, int32_t& nodeIndex);

  void findNearestSub(Vec3 point, int32_t nodeIndex, float& bestDist,
                      const KdTreeNode<KdTreePoint>*& bestDistNode) const;

  void findKNNSub(Vec3 targetPoint, int32_t numSerch, int32_t nodeIndex,
                  float& farDist,
                  std::vector<const KdTreePoint*>& points) const;
...
*/
template <typename KdTreePoint>
inline void KdTree<KdTreePoint>::construct(KdTreePoint* points,
                                           int32_t numPoints) {
...
*/
template <typename KdTreePoint>
inline size_t KdTree<KdTreePoint>::numNode() const {
  return nodes_.size();
}
...
*/
template <typename KdTreePoint>
inline int32_t KdTree<KdTreePoint>::constructSub(KdTreePoint* points,
                                                 int32_t numPoints,
  ...
  // その軸で二分する
  std::nth_element(
      points, points + midPoint, points + numPoints,
  ...
  node.right = constructSub(points + midPoint + 1, numPoints - midPoint - 1,
                            nodes, nodeIndex);
  node.axis = axis;
  // AABBの作成
...
*/
template <typename KdTreePoint>
INLINE KdTreePoint KdTree<KdTreePoint>::findNearest(Vec3 point,
                                                    float* minDist) const {
  // 空のツリーはそもそも探索しない
...
INLINE void KdTree<KdTreePoint>::findNearestSub(
    Vec3 point, int32_t nodeIndex, float& bestDist,
    const KdTreeNode<KdTreePoint>*& bestDistNode) const {
  //
  if (nodeIndex == -1) {
    return;
...
*/
template <typename KdTreePoint>
KdTreePoint KdTree<KdTreePoint>::findNearestBF(Vec3 point) const {
  // 空のツリーはそもそも探索しない
  AL_ASSERT_DEBUG(!nodes_.empty());
  //
...
float KdTree<KdTreePoint>::findKNN(
    Vec3 targetPoint, int32_t numSerch,
    std::vector<const KdTreePoint*>& points) const {
...
INLINE void KdTree<KdTreePoint>::findKNNSub(
    Vec3 targetPos, int32_t numSerch, int32_t nodeIndex, float& farDist,
    std::vector<const KdTreePoint*>& points) const {
  //
  if (nodeIndex == -1) {
    return;
  ...
  const auto setFarNodeToLast = [&]() {
    ...
    std::nth_element(points.begin(), points.begin() + points.size() - 1,
                     points.end(),
                     [&targetPos, &farDistSq, this](const KdTreePoint* lhs,
...
float KdTree<KdTreePoint>::findKNNBF(
    Vec3 targetPos, int32_t numSerch,
    std::vector<const KdTreePoint*>& points) const {
  // 探索する量よりもノードの数の方が少ない場合は失敗
  AL_ASSERT_DEBUG(numSerch < nodes_.size());
  //
  ...
  for (const NodeType& node : nodes_) {
    ...
    const auto setFarNodeToLast = [&targetPos, &points, this]() {
      std::nth_element(
          points.begin(), points.begin() + (points.size() - 1), points.end(),
          [&targetPos, this](const KdTreePoint* lhs, const KdTreePoint* rhs) {
...
*/
template <typename KdTreePoint>
INLINE float KdTree<KdTreePoint>::calcDist(const NodeType& node,
                                           Vec3 pos) const {
...
*/
template <typename KdTreePoint>
INLINE float KdTree<KdTreePoint>::calcDist(const KdTreePoint* point,
                                           Vec3 pos) const {
...
*/
template <typename KdTreePoint>
INLINE float KdTree<KdTreePoint>::calcDistSq(const NodeType& node,
                                             Vec3 pos) const {
...
*/
template <typename KdTreePoint>
INLINE float KdTree<KdTreePoint>::calcDistSq(const KdTreePoint* point,
                                             Vec3 pos) const {
