...
// setBRDF
//-------------------------------------------------
void AutoImportance::genDistribution(
    float coso,
    const BSDF* bsdf,
    ...
    Distribution2D* distribution)
{
    ...
    for (int32_t y = 0; y < GRID_SIZE; ++y)
    {
        ...
        for (int32_t x = 0; x < GRID_SIZE; ++x)
        {
            ...
            {
                // 微妙に摂動させる
                const float fx = ((float)x + rng.nextFloat()) / (float)(GRID_SIZE_HALF)-1.0f;
                const float fy = ((float)y + rng.nextFloat()) / (float)(GRID_SIZE_HALF)-1.0f;
