...
static void testBSDFcore(BSDFPtr bsdf)
{
    ...
    for (int32_t i = 0; i<128; ++i)
    {
        ...
        AL_ASSERT_ALWAYS(std::fabsf(statsRhd0.mean() / statsRhd1.mean() - 1.0f) < 0.01f);
    }
}
...
-------------------------------------------------
*/
Spectrum BSDF::estimateTotalRefrectance(const int32_t sampleNum, bool dontUseBsdfSample) const
{
#if 0
...
-------------------------------------------------
*/
Spectrum BSDF::estimateDirectionalRefrectance(
    Vec3 wo,
    int32_t sampleNum,
...
-------------------------------------------------
*/
AllBSDFList::AllBSDFList()
{
    bsdfs_.insert(std::make_pair("none", createObject<BSDF>("Lambertian")));
}
...
const BSDFPtr AllBSDFList::find(const std::string& bsdfName) const
{
    ...
    if (ite == bsdfs_.end())
    {
        ...
        */
        loggingWarning("BSDF [%s] isn't found.", bsdfName.c_str() );
        return BSDF::vantaBlack;
    }
    return ite->second;
}
...
Spectrum FresnelConductor::eval(float surfNormalDotIncomeDir) const
{
    ...
    const Spectrum rparl2 = (tmp - (2.0f * eta_ * cosi) + Spectrum(1.0f)) / (tmp + (2.0f * eta_ * cosi) + Spectrum(1.0f));
    const Spectrum tmp_f = eta_*eta_ + k_*k_;
    const Spectrum rperp2 = (tmp_f - (2.0f * eta_ * cosi) + Spectrum(cosi*cosi)) / (tmp_f + (2.f * eta_ * cosi) + Spectrum(cosi*cosi));
...
-------------------------------------------------
*/
INLINE FresnelDielectric::FresnelDielectric(float etai, float etat)
...
INLINE Spectrum FresnelDielectric::eval(float surfNormalDotIncomeDir) const
{
    ...
    else
    {
        ...
        const float rparl = (et * abscosi - ei * cost) / (et * abscosi + ei * cost);
        const float rperp = (ei * abscosi - et * cost) / (ei * abscosi + et * cost);
        // TODO: 怪しい
...
Lambertian::Lambertian(const ObjectProp& objectProp)
    :BSDF(objectProp)
{
    // HACK: reflectance以外も持つ場合があるから"reflectance"で検索掛けるようにした方がいいかも。
    ...
    {
        const auto spectrum = str2spectrum(reflectanceProp.attribute("value").asString("0.5 0.5 0.5"));
        baseColor_ = spectrum;
    }
    // srgbの場合
    ...
    Vec3 localWi) const
{
    ...
    Integrate[Sin[\[Theta]]*Cos[\[Theta]], {\[Phi], 0, 2*Pi}, {\[Theta], 0, Pi/2}] = Pi
    */
...
-------------------------------------------------
*/
Spectrum Lambertian::bsdfSample(
    Vec3 localWo,
    SamplerPtr sampler,
    _Out_ Vec3* localWi,
    _Out_ float* pdf) const
    ...
    Vec3 localWi ) const
{
    ...
    const Spectrum r = baseColor_ * INV_PI * (a_ + b_ * cosPhiiPhioClamped * sinalpha * tanbeta);
    return r;
}
...
-------------------------------------------------
*/
Spectrum MicrofacetBSDF::bsdf(
    Vec3 wo, Vec3 wi) const
{
...
-------------------------------------------------
*/
Spectrum MicrofacetBSDF::bsdfSample(
    Vec3 localWo,
    SamplerPtr sampler,
    _Out_ Vec3* localWi,
    _Out_ float* pdf) const
...
-------------------------------------------------
*/
Spectrum TorranceSparrow::bsdfSample(
    Vec3 localWo,
    SamplerPtr sampler,
    _Out_ Vec3* localWi,
    _Out_ float* pdf) const
    ...
    Vec3 localWi) const
{
    ...
    float spec = 1.0f / (4.0f * PI * ax * ay * sqrtf(cosTheta(localWi) * Vec3::dot(localWo, localWi)));
    spec *= exp(exponent);
    //
...
{
    return alphaG*alphaG * INV_PI / (sqr(NdotH*NdotH*(alphaG*alphaG - 1.0f) + 1.0f));
}
...
{
    return 2.0f / (1.0f + sqrtf(1.0f + alphaG*alphaG * (1.0f - Ndotv*Ndotv) / (Ndotv*Ndotv)));
}
    ...
    Vec3 localWi ) const
{
    ...
    const float F =
        useFresnel_ ?
        0.5f * powf(g - c, 2.0f) / powf(g + c, 2.0f) * (1.0f + powf(c*(g + c) - 1.0f, 2.0f) / pow(c*(g - c) + 1.0f, 2.0f)) :
        1.0f;
...
-------------------------------------------------
*/
Spectrum AshikhminShirley::bsdf(
    Vec3 localWo,
    Vec3 localWi ) const
{
    // 同じ面で無い場合は0
    ...
    const float norm_s = sqrt((nu_ + 1)*((isotropic_ ? nu_ : nv_) + 1)) / (8 * PI);
    const float n = isotropic_ ? nu_ : (nu_*sqr(HdotX) + nv_* sqr(HdotY)) / (1.0f - sqr(NdotH));
    const float rho_s = norm_s * F * powf(alMax(NdotH, 0.0f), n) / (HdotV * alMax(NdotV, NdotL));

    float rho_d = 28.0f / (23.0f * PI) * rd_ * (1 - pow(1 - NdotV / 2.0f, 5.0f)) * (1 - powf(1 - NdotL / 2.0f, 5.0f));
    if (coupled_diffuse_)
    {
        rho_d *= (1 - rs_);
    }
...
-------------------------------------------------
*/
Spectrum AshikhminShirley::bsdfSample(
    Vec3 localWo,
    SamplerPtr sampler,
    _Out_ Vec3* localWi,
    _Out_ float* pdf) const
    ...
    :BSDF(objectProp)
{
    ...
    roughness_ = objectProp.findChildBy("name", "roughness").asFloat(0.5f); // [0,1]
    specularTint_ = objectProp.findChildBy("name", "specularTint").asFloat(0.0f); // [0,1]
    anisotropic_ = objectProp.findChildBy("name", "anisotropic").asFloat(0.0f); // [0,1]
    sheen_ = objectProp.findChildBy("name", "sheen").asFloat(0.0f); // [0,1]
    sheenTint_ = objectProp.findChildBy("name", "sheenTint").asFloat(0.0f); // [0,1]
    clearcoat_ = objectProp.findChildBy("name", "clearcoat").asFloat(0.0f); // [0,1]
    clearcoatGloss_ = objectProp.findChildBy("name", "clearcoatGloss").asFloat(0.0f); // [0,1]
                                                                                      //
...
float DisneyBRDF::D_GTR2Aniso(Vec3 m, float MdotN2) const
{
    ...
    float denominator = alphaX_ * alphaY_ * sqr(sqr(HdotU / alphaX_) + sqr(HdotV / alphaY_) + MdotN2);
    ...
    Vec3 localWi ) const
{
    ...
    const Vec3 Cdlin = mon2lin(Vec3(baseColorRgb.r, baseColorRgb.g, baseColorRgb.b));
    const float Cdlum = 0.3f * Cdlin[0] + 0.6f * Cdlin[1] + 0.1f * Cdlin[2];

    //
    const Vec3 Ctint = Cdlum > 0.0f ? Cdlin / Cdlum : Vec3(1.0f);
    const Vec3 Cspec0 = alLerp(specular_ * 0.08f * alLerp(Vec3(1.0f), Ctint, specularTint_), Cdlin, metallic_);
    ...
    // specular
    const float Ds = GTR2_aniso(NdotH, Vec3::dot(H, X), Vec3::dot(H, Y), alphaX_, alphaY_);
    ...
    //
    const Vec3 rgb =
        (INV_PI * alLerp(Fd, ss, subsurface_) * Cdlin + Fsheen) * (1.0f - metallic_)
        + Gs*Fs*Ds +
...
-------------------------------------------------
*/
Spectrum DisneyBRDF::bsdfSample(
    Vec3 localWo,
    SamplerPtr sampler,
    _Out_ Vec3* localWi,
    _Out_ float* pdf) const
...
{
    //
    std::ifstream file(fileName.c_str(), std::ios_base::in | std::ifstream::binary);
...
-------------------------------------------------
*/
float MeasuredBSDF::pdf(
    Vec3 localWo,
    Vec3 localWi) const
{
    // TODO: 実装
    //http://people.csail.mit.edu/wojciech/BRDFDatabase/code/BRDFRead.cpp を参考に。
