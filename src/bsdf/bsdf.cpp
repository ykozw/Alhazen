  ...
  for (int32_t i = 0; i < 128; ++i) {
    ...
    for (int32_t sn = 0; sn < 1024 * 16; ++sn) {
      ...
      const auto same = [](const Spectrum& lhs, const Spectrum& rhs) {
        ...
        return (fabsf(d.r) < eps) && (fabsf(d.g) < eps) && (fabsf(d.b) < eps);
      };
    ...
    AL_ASSERT_ALWAYS(std::fabsf(statsRhd0.mean() / statsRhd1.mean() - 1.0f) <
                     0.01f);
  }
}
...
-------------------------------------------------
*/
Spectrum BSDF::estimateDirectionalRefrectance(Vec3 wo, int32_t sampleNum,
                                              bool dontUseBsdfSample) const {
#if 0
    const Vec2 dummyUV(0.0f, 0.0f);
    XorShift128 rng;
...
-------------------------------------------------
*/
void BlinnNDF::sample(Vec3 wo, SamplerPtr sampler, _Out_ Vec3* wi,
...
float Lambertian::pdf(Vec3 localWo, Vec3 localWi) const {
  ...
  Integrate[Sin[\[Theta]]*Cos[\[Theta]], {\[Phi], 0, 2*Pi}, {\[Theta], 0, Pi/2}]
  = Pi
  */
...
-------------------------------------------------
*/
Spectrum Lambertian::bsdfSample(Vec3 localWo, SamplerPtr sampler,
                                _Out_ Vec3* localWi, _Out_ float* pdf) const {
  float pdfHS;
...
-------------------------------------------------
*/
Spectrum OrenNayar::bsdfSample(Vec3 localWo, SamplerPtr sampler,
                               _Out_ Vec3* localWi, _Out_ float* aPdf) const {
  float pdfHS;
  *localWi = sampler->getHemisphereCosineWeighted(&pdfHS);
  // 裏面であればwiも裏返す
  if (localWo.z() < 0.) {
...
-------------------------------------------------
*/
Spectrum Mirror::bsdfSample(Vec3 localWo, SamplerPtr sampler,
                            _Out_ Vec3* localWi, _Out_ float* pdf) const {
  // 単純なreflect
...
-------------------------------------------------
*/
Spectrum Glass::bsdfSample(Vec3 localWo, SamplerPtr sampler,
                           _Out_ Vec3* localWi, _Out_ float* pdf) const {
  //
...
-------------------------------------------------
*/
MicrofacetBSDF::MicrofacetBSDF(const Spectrum& baseColor, FresnelTermPtr fTerm,
...
-------------------------------------------------
*/
Spectrum MicrofacetBSDF::bsdfSample(Vec3 localWo, SamplerPtr sampler,
                                    _Out_ Vec3* localWi,
                                    _Out_ float* pdf) const {
...
-------------------------------------------------
*/
Spectrum TorranceSparrow::bsdfSample(Vec3 localWo, SamplerPtr sampler,
                                     _Out_ Vec3* localWi,
                                     _Out_ float* pdf) const {
...
-------------------------------------------------
*/
Spectrum Ward::bsdfSample(const Vec3 localWo, SamplerPtr sampler,
                          _Out_ Vec3* localWi, _Out_ float* pdf) const {
  float pdfHS;
  *localWi = sampler->getHemisphereCosineWeighted(&pdfHS);
  // 裏面の場合
...
static float smithG_GGX(float Ndotv, float alphaG) {
  return 2.0f / (1.0f + sqrtf(1.0f + alphaG * alphaG * (1.0f - Ndotv * Ndotv) /
                                         (Ndotv * Ndotv)));
}
...
-------------------------------------------------
*/
Spectrum Walter::bsdfSample(Vec3 localWo, SamplerPtr sampler,
                            _Out_ Vec3* localWi, _Out_ float* pdf) const {
  float pdfHS;
  *localWi = sampler->getHemisphereCosineWeighted(&pdfHS);
  // 裏面の場合
...
Spectrum AshikhminShirley::bsdf(Vec3 localWo, Vec3 localWi) const {
  ...
  float rho_d = 28.0f / (23.0f * PI) * rd_ * (1 - pow(1 - NdotV / 2.0f, 5.0f)) *
...
-------------------------------------------------
*/
Spectrum AshikhminShirley::bsdfSample(Vec3 localWo, SamplerPtr sampler,
                                      _Out_ Vec3* localWi,
                                      _Out_ float* pdf) const {
  float pdfHS;
  *localWi = sampler->getHemisphereCosineWeighted(&pdfHS);
...
float DisneyBRDF::D_GTR2Aniso(Vec3 m, float MdotN2) const {
  ...
  float denominator = alphaX_ * alphaY_ *
...
-------------------------------------------------
*/
static float GTR2_aniso(float NdotH, float HdotX, float HdotY, float ax,
                        float ay) {
  //
...
-------------------------------------------------
*/
Spectrum DisneyBRDF::bsdfSample(Vec3 localWo, SamplerPtr sampler,
                                _Out_ Vec3* localWi, _Out_ float* pdf) const {
#if defined(USE_AUTO_IMPORTANCE)
...
MeasuredBSDF::MeasuredBSDF(const std::string& fileName) : BSDF(ObjectProp()) {
  ...
  // AL_ASSERT_DEBUG(n == BRDF_SAMPLING_RES_THETA_H * BRDF_SAMPLING_RES_THETA_D
...
-------------------------------------------------
*/
Spectrum MeasuredBSDF::bsdfSample(Vec3 localWo, SamplerPtr sampler,
                                  _Out_ Vec3* localWi, _Out_ float* pdf) const {
  // TODO: 実装
  assert(!"needs impliment");
  return Spectrum(0.0f);
