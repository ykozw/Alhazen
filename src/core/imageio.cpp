...
void Image::readHdr(const std::string& fileName) {
  ...
  loggingErrorIf((!isRadiance || !isRleRGBE),
                 "File format is not supported.[%s]", fileName.c_str());
  // "-Y 4096 +X 8192"
  int32_t width = 0;
  int32_t height = 0;
  std::getline(file, line);
  int32_t numScan = sscanf_s(line.c_str(), "-Y %d +X %d", &height, &width);
  loggingErrorIf(numScan != 2, "File format is not supported.[%s]",
                 fileName.c_str());
  resize(width, height);
  // 本体の読み込み
...
void Image::writeHdr(const std::string& fileName) const {
  ...
  for (int32_t y = 0; y < height; ++y) {
    ...
    // 行データを書く
    const std::array<std::vector<uint8_t>, 4> lines = {lineR, lineG, lineB,
                                                       lineE};
