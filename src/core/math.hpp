...
#if 1
  _mm_store_ss(&v, _mm_min_ss(_mm_max_ss(_mm_set_ss(v), _mm_set_ss(mn)),
                              _mm_set_ss(mx)));
  return v;
#else
  return v > mx ? mx : (v < mn ? mn : v);
...
-------------------------------------------------
*/
FORCE_INLINE static float alFmod(float x, float y) { return std::fmodf(x, y); }
...
struct ALIGN16 Matrix3x3 AL_FINAL {
 public:
  ...
  Matrix3x3(float e11, float e12, float e13, float e21, float e22, float e23,
...
struct ALIGN32 Vec3Pack8 AL_FINAL {
 ...
 public:
  ...
  INLINE void set(
      // xyz_が8回
      float* xyzs);
  INLINE void set(Vec3 v0, Vec3 v1, Vec3 v2, Vec3 v3, Vec3 v4, Vec3 v5, Vec3 v6,
                  Vec3 v7);
  INLINE void setZero();
