...
*/
template <typename T>
class RefArray AL_FINAL {
 public:
  RefArray() : values_(NULL), numElements_(0) {}
  RefArray(T* values, int32_t numElements)
      : values_(values), numElements_(numElements) {}
  int32_t size() const { return numElements_; }
  ...
  T* end() { return values_ + numElements_; }
  const T* begin() const { return values_; }
  const T* end() const { return values_ + numElements_; }
