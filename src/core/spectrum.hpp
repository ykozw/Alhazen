...
SpectrumSampled lerp(const SpectrumSampled& lhs, const SpectrumSampled& rhs,
                     float factor);
...
class SpectrumHerowavelength AL_FINAL {
 ...
 public:
  ...
  SpectrumRGB toRGB() const;
  Vec3 toXYZ() const;
  static SpectrumHerowavelength createFromWavelength(float w0, float w1,
                                                     float w2, float w3);
  static SpectrumHerowavelength createFromHerowavelength(
      float w0, bool isImportanceSamplingXYZ);
