...
void CounterStats::printStats(bool clearStats)
{
    ...
    const double elapseTimeInMs = std::chrono::duration_cast<std::chrono::milliseconds>(elapased).count();
    //
    logging("Counters");
    for(auto& ci : getCounterInfo())
    {
        ...
        if(MCountPerSec > 1.0)
        {
            logging("%10s: %2.1f M%s/sec", ci.title.c_str(), MCountPerSec, ci.unitName.c_str());
        }
        else if(KCountPerSec > 1.0)
        {
            logging("%10s: %2.1f K%s/sec", ci.title.c_str(), KCountPerSec, ci.unitName.c_str());
        }
        else
        {
            logging("%10s: %lld %s/sec", ci.title.c_str(), count, ci.unitName.c_str());
        }
    }
