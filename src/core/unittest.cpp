...
-------------------------------------------------
*/
void detail::registerTest(const char* testCategory, const char* testName,
                          std::function<void(void)> testFunc) {
  auto& tests = getTests();
...
void doTest() {
  ...
  for (const auto& testCategorys : tests) {
    ...
    for (const auto& test : testCategorys.second) {
      ...
      logging("UNIT TEST (%d/%d) [%s:%s]", testCount, numTestTotal,
              categoryName.c_str(), testName.c_str());
      ++testCount;
      //
  ...
  const int32_t elapseTimeInMs = int32_t(
      std::chrono::duration_cast<std::chrono::milliseconds>(elapseTimeRaw)
          .count());
  loggingWarningIf(elapseTimeInMs > 3000, "UnitTest is too waste time(%dms).",
                   elapseTimeInMs);
}
