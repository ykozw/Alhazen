...
*/
bool
PTSurfaceIntegrator::preRendering(const SceneGeometory& scene,
...
*/
static float
misBalanceHeuristic(float fPdf, float gPdf)
{
...
*/
Spectrum
PTSurfaceIntegrator::estimateOneLight(const SceneGeometory& scene,
                                      const Intersect& isect,
                                      const OrthonormalBasis<>& local,
                                      const Vec3 localWo,
                                      const LightPtr& light,
    ...
    */
    ...
    const auto lightSampling = [](SamplerPtr sampler,
        ...
        const Spectrum spectrum =
          reflectance * emission * std::fabsf(localWi.z()) * weight / pdfLight;
