...
class Light : public SceneObject {
 public:
  ...
  virtual Spectrum sampleLe(SamplerPtr sampler, Vec3 targetPos,
                            _Out_ Vec3* samplePos, _Out_ float* pdf) const = 0;
...
class RectangleLight AL_FINAL : public Light {
 public:
  ...
  Spectrum emission(Vec3 targetPos, Vec3 dir) const override;
  float pdf(Vec3 targetPos, Vec3 dir) const override;
  Spectrum sampleLe(SamplerPtr sampler, Vec3 targetPos, _Out_ Vec3* samplePos,
                    _Out_ float* pdf) const override;
  float area() const;
