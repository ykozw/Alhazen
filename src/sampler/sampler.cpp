...
-------------------------------------------------
*/
uint32_t Sampler::dimention() const
{
    return dimention_;
}
...
-------------------------------------------------
*/
void Sampler::setDimention(uint32_t dimention)
{
...
-------------------------------------------------
*/
Vec3 Sampler::getHemisphereCosineWeighted(_Out_ float* pdf)
{
    //
...
float SamplerHalton::get1d()
{
    ...
    const float v = std::fmodf(radicalInverseFast(dimention_, sampleNo_) + offset,1.0f);
...
SphericalFibonacci::SphericalFibonacci(const ObjectProp& objectProp)
{
    ...
    // シード設定する
    const int32_t seed = objectProp.findChildBy("name", "seed").asInt(123456789);
    rng_.setSeed(seed);
}
...
-------------------------------------------------
*/
bool SphericalFibonacci::isCosineWeighted() const
{
    return true;
}
