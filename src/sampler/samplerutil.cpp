...
float estimateStarDiscrepancy(const std::vector<Vec2>& srcSamples) {
  ...
  for (int32_t i = 0; i < numSample; ++i) {
    ...
    const auto beginX = std::lower_bound(samples.begin(), samples.end(), mn,
                                         [](const Vec2& lhs, const Vec2& rhs) {
                                           if (lhs.x() < rhs.x()) {
                                             return true;
                                           } else if (lhs.x() > rhs.x()) {
    ...
    const auto lastX = std::upper_bound(samples.begin(), samples.end(), mn,
                                        [](const Vec2& lhs, const Vec2& rhs) {
                                          if (lhs.x() < rhs.x()) {
                                            return true;
                                          } else if (lhs.x() > rhs.x()) {
    ...
    maxDiscrepancy =
        std::max((float(count) / float(samples.size()) - area), maxDiscrepancy);
  }
