...
bool loadOtx(const std::string& fileName, LensSet& lensSet)
{
    ...
    while (std::getline(file, buffer))
    {
        ...
        else if (strcmp(tag, "APE") == 0)
        {
            ...
            if (sscanf(buffer.c_str(), "  APE %d %f %f %f %f %f %d %d %d",
                       &numApe, &apeX, &apeY,
                       &apes[0], &apes[1], &apes[2],
                       &dummys[0], &dummys[1], &dummys[2]) != 9)
            {
    ...
    // TODO: 曲率が強すぎて、撮像素子から見えない点があるような状態がないかのチェック
...
RealSensor::RealSensor(const ObjectProp& objectProp)
{
    ...
    upDir_ *= tf;
    // レンズの作成
    const std::string lensFileName = objectProp.findChildBy("name","otxfile").asString("test.otx");
    loadOtx(lensFileName, surfs_);
    // オートフォーカス
    const float aimWorkingDistance = objectProp.findChildBy("name", "workingDistance").asFloat(1000.0f);
...
void RealSensor::calcExitPupile()
{
    ...
    for (int32_t py = 0, h = img.height(); py <= h / 2; py += pixelSkipY)
    {
        ...
        {
            const float xOnFilmInMM = ((float)px / (float)(w - 1) - 0.5f) * filmWidthInMM_;
            const float yOnFilmInMM = ((float)py / (float)(h - 1) - 0.5f) * filmHeightInMM_;
            AABB2D exitPupil;
            for (int32_t gy = 0; gy < gridSize; ++gy)
            {
                ...
                for (int32_t gx = 0; gx < gridSize; ++gx)
                {
                    ...
                    const float blx = backLensApeX * (2.0f * (float)gx / (float)(gridSize - 1) - 1.0f);
                    const float bly = backLensApeY * (2.0f * (float)gy / (float)(gridSize - 1) - 1.0f);
...
// rは負の値も受け付ける
//-------------------------------------------------
INLINE bool intersectSphereAsLens(
    const Ray& ray,
    float oz,
    float r,
    IntersectLens& isect )
    ...
    _Out_ Ray* outRay) const
{
    ...
    for (int32_t i = lensNumM1; i >= 0; --i)
    {
        ...
        else
        {
            ...
            {
                // レンズに交差しない場合は失敗
                if (!intersectSphereAsLens(ray, lens.z - lens.radius, lens.radius, isect))
                {
                    return false;
                }
            }
...
float RealSensor::workingDistance(const float imageSensorOffset) const
{
    ...
    rayDir.normalize();
    Ray outRay;
    if (!lensTrace(0.0f, 0.0f, rayDir, WAVE_LENGTH_D, imageSensorOffset, NULL, &outRay))
    {
        AL_ASSERT_DEBUG(false);
    }
...
float RealSensor::sensorOffsetRangeUpperBound() const
{
    ...
    for (int32_t loopNo = 0; loopNo<32; ++loopNo)
    {
        ...
        const float wd = workingDistance(imageSensorOffsetTmp);
        // 十分近づけば終了
        if (fabsf(1.0f - sensorOffsetHigh / sensorOffsetLow) < 0.0001f && wd > 0.0f )
        {
...
{
    //
    const Vec2 lensSample(0.0f,0.0f); // TODO: RealLensにSamplerが引数で渡されるのを待つ
    ++sampleNo_; // TOOD: サンプル番号は呼び出し元からもらうようにする。
    const float xOnFilmInMM = (0.5f - imageX * invScreenWidth_) * filmWidthInMM_;
    const float yOnFilmInMM = (imageY * invScreenHeight_ - 0.5f) * filmHeightInMM_;
    /*
    フィルム上から後玉上の射出瞳上へのレイのサンプル位置を決定する
    HACK: 一様に飛ばした上でpdfなどは用意していないが、本当はこの方法は正しくない。
    とりあえず絵が出るまではこの方法を使う
    */
