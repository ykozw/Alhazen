  ...
  while (std::getline(file, buffer)) {
    ...
    */
    ...
    // 口径
      ...
      if (sscanf(buffer.c_str(), "  APE %d %f %f %f %f %f %d %d %d", &numApe,
                 &apeX, &apeY, &apes[0], &apes[1], &apes[2], &dummys[0],
                 &dummys[1], &dummys[2]) != 9) {
...
class RealSensor AL_FINAL : public Sensor {
 ...
 private:
  ...
  bool lensTrace(float xOnFilmInMM, float yOnFilmInMM, Vec3 dir,
                 float waveLength, float imageSensorOffset,
                 _Out_ std::vector<Vec3>* points, _Out_ Ray* outRay) const;
...
#pragma omp parallel for schedule(dynamic, 1)
        for (int32_t gx = 0; gx < gridSize; ++gx) {
          ...
          const float blx =
              backLensApeX * (2.0f * (float)gx / (float)(gridSize - 1) - 1.0f);
          const float bly =
              backLensApeY * (2.0f * (float)gy / (float)(gridSize - 1) - 1.0f);
          const float blz = backLensDist - imageSensorOffset_;
          Vec3 rayDir(blx - xOnFilmInMM, bly - yOnFilmInMM, blz);
          rayDir.normalize();
          Ray outRay;
          // D線のみで入射瞳は計算される
          if (!lensTrace(xOnFilmInMM, yOnFilmInMM, rayDir, WAVE_LENGTH_D,
                         imageSensorOffset_, NULL, &outRay)) {
            continue;
          }
#pragma omp critical
...
-------------------------------------------------
*/
INLINE bool intersectSphereAsLens(const Ray& ray, float oz, float r,
                                  IntersectLens& isect) {
  float t;
...
-------------------------------------------------
*/
INLINE void intersectPlaneAsStopPlane(Vec3 rayOrig, Vec3 rayDir,
                                      const float dist, _Out_ Vec3* hitPos) {
...
-------------------------------------------------
*/
bool RealSensor::lensTrace(float xOnFilmInMM, float yOnFilmInMM, Vec3 dir,
                           float waveLength, float imageSensorOffset,
  ...
  for (int32_t i = lensNumM1; i >= 0; --i) {
    ...
    // レンズの場合
      ...
      if (lens.invRadius != 0.0f) {
        // レンズに交差しない場合は失敗
        if (!intersectSphereAsLens(ray, lens.z - lens.radius, lens.radius,
                                   isect)) {
          return false;
        }
      }
...
float RealSensor::workingDistance(const float imageSensorOffset) const {
  ...
  rayDir.normalize();
  Ray outRay;
  if (!lensTrace(0.0f, 0.0f, rayDir, WAVE_LENGTH_D, imageSensorOffset, NULL,
                 &outRay)) {
    AL_ASSERT_DEBUG(false);
  }
...
Ray RealSensor::generateRay(float imageX, float imageY, float& pdf) const {
  ...
  */
  ...
  Vec3 rayDir(sampleOnLens.x() - xOnFilmInMM, sampleOnLens.y() - yOnFilmInMM,
              sampleOnLensZ);
  rayDir.normalize();
  Ray outRay;
  if (!lensTrace(xOnFilmInMM, yOnFilmInMM, rayDir, WAVE_LENGTH_D,
