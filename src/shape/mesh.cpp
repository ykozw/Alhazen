            ...
            const char* readBuffer)
{
    ...
    const ReadFFormatFunc readFaceFuncs[4] = {
        readFFormat1, readFFormat2, readFFormat3, readFFormat4
    };
    if (readFaceFuncs[prevIndex](readBuffer,
                                 &vertexIndex[0],
                                 &vertexIndex[1],
                                 &vertexIndex[2],
                                 &uvIndex[0],
...
*/
bool
checkAsVertexNormal(Mesh& this_,
                    int32_t& currentMaterialId,
                    MaterialIds& materialIds,
                    const char* readBuffer)
{
...
*/
bool
Mesh::loadFromoObj(const std::string& fileName)
{
        ...
        // フォーマットが変わっている場合は全ての場合について再度チェック
            ...
            for (funcNo = 0; funcNo < NUM_FUNC; ++funcNo) {
                if (checkLineFuncs[funcNo](
                      *this, currentMaterialId, materialIds_, readBuffer)) {
                    // logging("OBJ: Change line format[%d]->[%d]",
...
*/
void
Mesh::applyTransform(const Transform& transform)
{
    for (auto& v : vs) {
...
Mesh::recalcBound()
{
    ...
    logging("AABB(%.2f,%.2f,%.2f)-(%.2f,%.2f,%.2f)",
            mn.x(),
            mn.y(),
            mn.z(),
            mx.x(),
