 ...
 */
void
PlyFile::loadPly(const char* filePath)
{
    //
 ...
 */
bool
PlyFile::readHeader(FILE* file, PlyHeader& header) const
{
    if (file == nullptr) {
        return false;
    }
    ...
    for (;;) {
        ...
        //
        if (sscanf(
              buffer.data(), "element %[^ ] %[^ ]", tmp0.data(), tmp1.data()) ==
            2) {
 ...
 */
bool
PlyFile::readBodyAsAscii(FILE* file,
 ...
 */
bool
PlyFile::readBodyAsBinary(FILE* file,
                          const PlyFile::PlyHeader& header,
                          PlyBody& body) const
{
    // 頂点データをロード
