...
bool PlyFile::readHeader(FILE* file, PlyHeader& header) const {
  ...
  for (;;) {
    ...
    //
    if (sscanf(buffer.data(), "element %[^ ] %[^ ]", tmp0.data(),
               tmp1.data()) == 2) {
...
/*
 ...
 -------------------------------------------------
 */
bool PlyFile::readBodyAsAscii(FILE* file, const PlyFile::PlyHeader& header,
                              PlyBody& body) const {
  ...
  for (int32_t vi = 0; vi < header.numVertex; ++vi) {
    ...
    if (sscanf(buffer.data(), "%[^ ] %[^ ] %[^ ]", tmp0.data(), tmp1.data(),
               tmp2.data()) == 3) {
  ...
  for (int32_t fi = 0; fi < header.numFace; ++fi) {
    ...
    if (sscanf(buffer.data(), "%[^ ] %[^ ] %[^ ] %[^ ]", tmp0.data(),
               tmp1.data(), tmp2.data(), tmp3.data()) == 4) {
...
/*
 ...
 -------------------------------------------------
 */
bool PlyFile::readBodyAsBinary(FILE* file, const PlyFile::PlyHeader& header,
                               PlyBody& body) const {
  // 頂点データをロード
  auto& vtxs = body.vertex_;
  for (int32_t vi = 0; vi < header.numVertex; ++vi) {
