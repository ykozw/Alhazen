...
-------------------------------------------------
*/
INLINE bool BoxShape::intersect(const Ray& ray,
...
#else
  //
  const auto intersectFace = [](const Ray& ray, const Face& face,
                                _Inout_ Intersect* isect) {
    const auto& vs = face.vs;
    const Vec3 n = face.n;
    // HACK: UVは適当
    const Vec2 uv(0.0f);
    return intersectTriangle(ray, vs[0], vs[1], vs[2], n, n, n, uv, uv, uv,
                             isect) ||
           intersectTriangle(ray, vs[2], vs[1], vs[3], n, n, n, uv, uv, uv,
                             isect);
  };
  bool isHit = false;
...
-------------------------------------------------
*/
INLINE bool RectangleShape::intersect(const Ray& ray,
                                      _Inout_ Intersect* isect) const {
  const bool hit0 = intersectTriangle(ray, verts_[0], verts_[1], verts_[2], n_,
                                      n_, n_, uvs_[0], uvs_[1], uvs_[2], isect);
  const bool hit1 = intersectTriangle(ray, verts_[2], verts_[1], verts_[3], n_,
                                      n_, n_, uvs_[1], uvs_[2], uvs_[3], isect);
  if (!hit0 && !hit1) {
    return false;
  }
  isect->bsdf = bsdf_;
