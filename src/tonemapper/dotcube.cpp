...
public:
    ...
    Vec3 sample(float r, float g, float b) const
    {
        ...
        // X方向
        const Vec3 v00 =
          pixel(ir, ig, ib) * (1.0f - fr) + pixel(ur, ig, ib) * (fr);
        const Vec3 v01 =
          pixel(ir, ug, ib) * (1.0f - fr) + pixel(ur, ug, ib) * (fr);
        const Vec3 v02 =
          pixel(ir, ig, ub) * (1.0f - fr) + pixel(ur, ig, ub) * (fr);
        const Vec3 v03 =
          pixel(ir, ug, ub) * (1.0f - fr) + pixel(ur, ug, ub) * (fr);
        // Y方向
...
*/
bool
Dotcube::process(const Image& src, ImageLDR& dst) const
{
    //
