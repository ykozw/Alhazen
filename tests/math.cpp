...
TEST_CASE("Vec2", "Math")
{
    ...
    {
        // 同値
        AL_ASSERT_ALWAYS(Vec2(1.0f, 2.0f) == Vec2(1.0f, 2.0f));
        // 非同値
        AL_ASSERT_ALWAYS(Vec2(0.0f, 2.0f) != Vec2(1.0f, 2.0f));
        AL_ASSERT_ALWAYS(Vec2(1.0f, 0.0f) != Vec2(1.0f, 2.0f));
        // zwは関係がないので同値になる
...
TEST_CASE("Vec3", "Math")
{
    ...
    {
        // 同値
        AL_ASSERT_ALWAYS(Vec3(1.0f, 2.0f, 3.0f) == Vec3(1.0f, 2.0f, 3.0f));
        // 非同値
        AL_ASSERT_ALWAYS(Vec3(0.0f, 2.0f, 3.0f) != Vec3(1.0f, 2.0f, 3.0f));
        AL_ASSERT_ALWAYS(Vec3(1.0f, 0.0f, 3.0f) != Vec3(1.0f, 2.0f, 3.0f));
        AL_ASSERT_ALWAYS(Vec3(1.0f, 2.0f, 0.0f) != Vec3(1.0f, 2.0f, 3.0f));
        // wは関係がないので同値になる
    ...
    // normalize(2)
    {
        ...
        const Vec3 v1 = v0.normalized();
        AL_ASSERT_ALWAYS(sameV(v1.x(), 1.0f) && (v1.y() == 0.0f) &&
                         (v1.z() == 0.0f));
        AL_ASSERT_ALWAYS(v1.isNormalized());
    }
    // scale
...
-------------------------------------------------
*/
TEST_CASE("Matrix3x3_next", "Math")
{
    // transpose
    {
        const Matrix3x3 m0(Vec3(0.0f, 1.0f, 2.0f),
                           Vec3(3.0f, 4.0f, 5.0f),
                           Vec3(6.0f, 7.0f, 8.0f));
